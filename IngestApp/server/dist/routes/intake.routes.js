...
const fieldSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['text', 'textarea', 'number', 'select', 'checkbox']),
    required: zod_1.z.boolean().optional().default(false),
    placeholder: zod_1.z.string().optional(),
    options: zod_1.z.array(zod_1.z.string()).optional(), // for selects
});
...
const submitIntakeSchema = zod_1.z.object({
...
// ---------- Helpers ----------
const parseFieldsJson = (fieldsJson) => {
    try {
        const parsed = JSON.parse(fieldsJson);
        return zod_1.z.array(fieldSchema).parse(parsed);
    }
    catch (err) {
...
router.post('/templates', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const template = await prisma_1.prisma.intakeFormTemplate.create({
            data: {
                name,
                description,
                isActive,
...
router.get('/templates/active', async (_req, res) => {
    try {
        const templates = await prisma_1.prisma.intakeFormTemplate.findMany({
            where: { isActive: true },
...
router.get('/appointment/:appointmentId', auth_1.authMiddleware, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Not authenticated' });
        }
        ...
        // choose template
        const template = await prisma_1.prisma.intakeFormTemplate.findFirst({
            where: { isActive: true },
...
router.post('/submit', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const submission = await prisma_1.prisma.intakeSubmission.upsert({
            where: {
