...
const createAppointmentSchema = zod_1.z.object({
    clientId: zod_1.z.string().optional(), // optional: filled automatically for CLIENT
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const client = await prisma_1.prisma.user.findUnique({
            where: { id: clientId },
        });
        if (!client) {
        ...
        const overlapping = await prisma_1.prisma.appointment.findFirst({
            where: {
                practitionerId,
                startsAt: { lt: end },
                endsAt: { gt: start },
